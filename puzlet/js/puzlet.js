...
(function() {
  var AceIdentifiers, AceModes, ArrayMath, AxesLabels, BlabCoffee, BlabPlotter, BlabPrinter, CodeNode, CodeNodeComment, CodeNodeFunction, CodeNodeSource, CoffeeEvaluator, CoffeeResource, ComplexMath, CssResourceInline, CssResourceLinked, EvalBoxPlotter, FavIcon, Figure, GithubRibbon, HtmlResource, JsResourceInline, JsResourceLinked, JsonResource, Loader, MathJaxProcessor, NumericFunctions, Page, Resource, ResourceInline, Resources, ScalarMath, TypeMath, getBlabFromQuery, init, initAce,
  ...
  */
  ...
  Loader = (function() {
    ...
    */
    ...
    Loader.prototype.aceResources1 = [
      {
        ...
        return _this.loadResourceList(function() {
          return _this.loadHtmlCss(function() {
            return _this.loadScripts(function() {
              return _this.loadAce1(function() {
                return _this.loadAce2(function() {
                  return _this.done();
                });
              });
            });
    ...
    Loader.prototype.loadAce1 = function(callback) {
      var _this = this;
      this.resources.add(this.aceResources1);
  ...
  Page = (function() {
    ...
    Page.prototype.ready = function(resources) {
      ...
      new FavIcon;
      this.processCodeNodes();
      return new GithubRibbon(this.container, this.blab);
    };

    Page.prototype.processCodeNodes = function() {
      var codeNodes, filename, findCode, n, node, resource, _i, _len,
        _this = this;
      codeNodes = $("div[data-file]");
      findCode = function(filename) {
        var resource, url, _i, _len, _ref;
        _ref = _this.resources.resources;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          resource = _ref[_i];
          url = resource.url;
          if (url === filename) {
      ...
      for (_i = 0, _len = codeNodes.length; _i < _len; _i++) {
        n = codeNodes[_i];
        node = $(n);
        filename = node.attr("data-file");
        resource = findCode(filename);
        if (resource) {
          new CodeNode(node, resource);
        }
      }
      return initAce();
    };
  ...
  CodeNode = (function() {
    ...
    CodeNode.prototype.html = function() {
      var codeNodeFilename, codeNodeLanguage, codeNodeTextAreaContent, filename, html, id;
  ...
  AceModes = (function() {
    ...
    function AceModes() {
      var Mode, lang, mode, _ref;
      this.ace3 = false;
      this.names = {
        python: "ace/mode/python",
        octave: this.ace3 ? "ace/mode/octave" : "ace/mode/matlab",
      ...
      for (lang in _ref) {
        ...
        if (this.ace3) {
          ...
          this.modes[lang] = new Mode();
        } else {
          this.modes[lang] = mode;
        }
      }
    }

    AceModes.prototype.hasMode = function(lang) {
      return this.names.hasOwnProperty(lang);
    };

    AceModes.prototype.getMode = function(lang) {
      if (this.hasMode(lang)) {
        return this.modes[lang];
      } else {
        return null;
      }
    };
  ...
  CodeNodeSource = (function() {

    function CodeNodeSource(codeNodeContainer, idxInPage) {
      var mode;
      this.codeNodeContainer = codeNodeContainer;
      this.idxInPage = idxInPage;
      this.outer = this.codeNodeContainer.find(".code_node_editor_container");
      this.container = this.codeNodeContainer.find(".code_node_editor");
      this.textarea = this.codeNodeContainer.find(".code_node_textarea");
      this.id = this.container.attr("id");
      this.lang = this.container.data("lang");
      this.editor = ace.edit(this.id);
      this.editor.setTheme("ace/theme/textmate");
      if (this.lang) {
        mode = $pz.aceModes.getMode(this.lang);
      }
      if (mode) {
    ...
    CodeNodeSource.prototype.initRenderer = function() {
      var id,
        _this = this;
    ...
    CodeNodeSource.prototype.initFont = function() {
      var char, css;
      this.container.addClass("pz_ace_editor");
      css = {
    ...
    CodeNodeSource.prototype.setHeight = function() {
      ...
      this.container.css("height", heightStr);
      this.outer.css("height", heightStr);
      return this.editor.resize();
    };

    CodeNodeSource.prototype.customRendering = function() {
      var onBlur, onFocus,
        _this = this;
    ...
    CodeNodeSource.prototype.render = function() {
      ...
      commentNodes = this.container.find(".ace_comment");
      ...
      identifiers = this.container.find(".ace_identifier");
      this.functions = (function() {
        var _j, _len1, _results;
        _results = [];
        for (_j = 0, _len1 = identifiers.length; _j < _len1; _j++) {
          i = identifiers[_j];
          if (l = AceIdentifiers.link($(i).text())) {
    ...
    CodeNodeSource.prototype.restoreCode = function() {
    ...
    CodeNodeSource.prototype.mouseUpHandler = function() {
      var href, target;
    ...
    CodeNodeSource.prototype.focus = function() {
      return this.editor.focus();
    };

    CodeNodeSource.prototype.session = function() {
      if (this.editor) {
    ...
    CodeNodeSource.prototype.code = function() {
      return this.session().getValue();
    };

    CodeNodeSource.prototype.show = function(show) {
      return this.outer.css("display", show ? "" : "none");
    };

    CodeNodeSource.prototype.showCode = function(show) {
    ...
    return CodeNodeSource;

  })();

  AceIdentifiers = (function() {

    function AceIdentifiers() {}

    AceIdentifiers.links = {};

    AceIdentifiers.registerLinks = function(links) {
      var identifier, link, _results;
      _results = [];
      for (identifier in links) {
        link = links[identifier];
        _results.push(AceIdentifiers.links[identifier] = link);
      }
      return _results;
    };

    AceIdentifiers.link = function(name) {
      return AceIdentifiers.links[name];
    };

    return AceIdentifiers;
  ...
  initAce = function() {
    ...
    $pz.codeNode = {};
    _results = [];
    for (idxInPage = _i = 0, _len = codeNodeContainers.length; _i < _len; idxInPage = ++_i) {
